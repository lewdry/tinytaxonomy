<script lang="ts">
	...
	// Search state
	let searchTerm = "";
	let searchMatches = new Set<number>();
	...
	// Re-render when search term changes
	...
	function handleSearch(term: string) {
		if (!term.trim()) {
			searchMatches.clear();
			if (data && svgElement) renderTree(data);
			return;
		}
		...
		// Helper to find matches and collect paths
		function findMatches(node: TaxonomyNode, path: number[]) {
			const nodeText =
				(node.name || "") +
				" " +
				(node.fullText || "") +
				...
				(node.clusterLabel || "") +
				" " +
				(node.clusterKeywords?.join(" ") || "");
			if (nodeText.toLowerCase().includes(lowerTerm)) {
				if (node.id) matches.add(node.id);
			...
			if (node.children) {
				...
				node.children.forEach((child) => findMatches(child, newPath));
			}
		}
	...
	function renderTree(hierarchyData: TaxonomyNode) {
		...
		const g = svg
			...
			.attr(
				...
				layout === "tree"
					? "translate(100,0)"
					: `translate(${width / 2},${dynamicHeight / 2})`,
			);

		// Compute color scale for cluster height if available
		const internalNodes = root
			.descendants()
			.filter((d: any) => d.children && Array.isArray(d.children));
		const heights = internalNodes
			.map((d: any) => d.data?.height)
		...
		const branchKeys = topLevelChildren.map((c: any) =>
			normalizeKey(
				c.data?.clusterLabel ??
					c.data?.name ??
					String(c.data?.id ?? Math.random()),
			),
		);
		const branchColor = d3
			.scaleOrdinal<string>(d3.schemeTableau10)
			.domain(branchKeys);
		...
		const linkSel = g
			...
			.attr(
				"class",
				"link fill-none stroke-gray-300 stroke-[1.5px] opacity-70",
			)
			...
			.attr("stroke", (d: any) => {
				...
				const anc =
					d.source
						.ancestors()
						.reverse()
						.find((a: any) => a.depth === 1) ?? d.source;
				const key = normalizeKey(
					anc?.data?.clusterLabel ??
						anc?.data?.name ??
						String(anc?.data?.id ?? "root"),
				);
			...
			.attr("stroke-width", (d: any) => {
				...
				if (
					...
					const norm =
						(parent.height - heightMin) /
						Math.max(1e-6, heightMax - heightMin);
		...
		node.each(function (this: SVGGElement, d: any) {
			...
			const isMatch = d.data.id && searchMatches.has(d.data.id);
			...
			if (d.children) {
				...
				g.append("path")
					...
					.attr("fill", () => {
						if (isMatch) return "#ffeb3b"; // Highlight match
					...
					.attr("stroke", isMatch ? "#000" : "#1f2937")
					.attr("stroke-width", isMatch ? 2 : 0.4)
					.on("dblclick", (evt: any) => {
				...
				g.append("circle")
					...
					.attr("stroke", isMatch ? "#000" : "none")
					.attr("stroke-width", isMatch ? 1.5 : 0)
					.on("click", (evt: any) => {
		...
		node.append("text")
			.attr("dy", 3)
			.attr("x", (d: any) => (d.children ? -12 : 10))
			.style("text-anchor", (d: any) => (d.children ? "end" : "start"))
			.attr("class", "text-xs font-semibold fill-gray-700")
			.style("font-weight", (d: any) =>
				searchMatches.has(d.data.id) ? "bold" : "normal",
			)
			.style("fill", (d: any) =>
				searchMatches.has(d.data.id) ? "#000" : "#374151",
			)
			.text((d: any) => {
				if (d.children) {
					...
					const cl =
						d.data?.clusterLabel ??
						d.data?.clusterKeywords?.join(" / ") ??
						d.data.name;
					const H =
						typeof d.data?.height === "number"
							? ` • H:${d.data.height.toFixed(2)}`
							: "";
					return `${cl}${H}`;
				}
				return d.data.name;
			...
			.text((d: any) => {
				...
				if (!d.children)
					return d.data.fullText || d.data.name || "leaf";
				...
				const sample =
					...
					d
						.leaves()
						.map((l: any) => l.data?.fullText ?? l.data?.name ?? "")
						.slice(0, 5);
				const sampleText =
				...
				const keywords =
					d.data?.clusterKeywords && d.data.clusterKeywords.length
						? `Keywords: ${d.data.clusterKeywords.join(", ")}`
						: "";
				const label = d.data?.clusterLabel
					? `Label: ${d.data.clusterLabel}`
					: "";
		...
		node.filter(
			(d: any) =>
				d.children && d.data && typeof d.data.height === "number",
		)
			.append("rect")
			.attr("x", (d: any) =>
				d.children ? -Math.round(maxBarWidth / 2) : 8,
			)
			.attr("y", 12)
		...
		node.filter(
			...
			.attr("width", (d: any) => {
				...
				// normalize to 0..1
				const norm =
					(h - heightMin) / Math.max(1e-6, heightMax - heightMin);
		...
		const perRow = Math.max(
			1,
			Math.min(visibleBranches.length, Math.floor(420 / swatchWidth)),
		);
		...
		if (visibleBranches.length) {
			visibleBranches.forEach((key: any, i: number) => {
				...
				const label = content
					...
					.text(key.length > 22 ? key.substring(0, 20) + "…" : key)
					.style("cursor", "pointer");
				...
				label
					...
					.on("mouseout", onOut)
					.on("click", () => {
						const m = topLevelChildren.find(
							(c: any) =>
								normalizeKey(
									c.data?.clusterLabel ??
										c.data?.name ??
										String(c.data?.id ?? "root"),
								) === key,
						);
						if (m && m.data && m.data.id) {
							if (collapsed.has(m.data.id))
								collapsed.delete(m.data.id);
							else collapsed.add(m.data.id);
							renderTree(hierarchyData);
						}
					});
			});
			...
			content
				...
				.attr("y", legendLineY)
				.attr("class", "text-xs fill-gray-400")
				.text("• Leaf = original item (circle)");
			legendLineY += 24;
			content
				.append("text")
				.attr("x", 10)
				...
				.attr("class", "text-xs fill-gray-400")
				.text(
					"• Color = semantic topic family (branches share a color)",
				);
			legendLineY += 24;
			content
				.append("text")
				...
				.attr("class", "text-xs fill-gray-400")
				.text(
					"• Cluster = merged group (diamond); label = learned concept (TextRank); tooltip shows examples",
				);
			legendLineY += 24;
			...
			content
				...
				.attr("class", "text-xs fill-gray-400")
				.text(
					"• Bar width = merge distance (H); distance ↑ = weaker relationship",
				);
			legendLineY += 24;
			content
				.append("text")
				...
				.attr("class", "text-xs fill-gray-400")
				.text(
					"• Double-click a diamond to collapse/expand that branch",
				);
		} else {
...
<div
	...
	class="w-full h-full border bg-white rounded-lg overflow-hidden relative"
>
	<div class="absolute top-4 left-4 z-10">
		<input
			...
			class="px-2 py-1 text-sm border rounded shadow-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white/90"
		/>
	...
	></svg>
	<div
		class="absolute top-4 right-4 text-xs text-gray-400 pointer-events-none bg-white/60 rounded px-2 py-1"
	>
