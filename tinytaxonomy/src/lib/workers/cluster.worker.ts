...
import { agnes } from 'ml-hclust'; // FIX: Use direct named import for clean ESM resolution
...
// 3. The 'agnes' clustering function is now directly available via import.
...
function convertToD3(node: any, labels: string[]): any {
	// Leaf node (index is defined)
	if (node.index !== undefined) {
        // FIX: Add robust guard against undefined label
	...
	// Cluster node (children are defined)
	return {
		name: '', 
